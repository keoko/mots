...
* 2025
** 2025/Nov/20
*** feat: add landing page for first-time visitors
Educational landing page with FAR explanation, feature cards, and learning path visualization. Shows once on first visit (localStorage detection), then goes directly to topic selection.

Files: landing.js (new), game.js, ui.js, styles.css

** 2025/Nov/19
*** feat: implement cached global leaderboard with local-first sync
Added localStorage caching for global leaderboards with 5-minute freshness window:
- Cache-first strategy: instant display from cache, background refresh if stale
- Optimistic updates: cache updated immediately on score submission
- Freshness indicator shows sync time (relative + absolute) with manual refresh button
- Visual states: ✓ fresh (green), ⚠️ stale (yellow), ⏳ loading (blue)
- Background refresh on app launch and when coming back online
- Old caches (>7 days) automatically cleaned up
- Leaderboard stays visible during refresh (no blocking "Loading..." state)

Files: storage.js, leaderboard-api.js, sync.js, ui.js, styles.css

** 2025/Nov/17
*** refactor: improve leaderboard share UX with inline badges
- Replaced "Share with All" button with inline 📤 badge on best score row
- Badge only shows if score qualifies for global top 10
- Fixed current-rank highlighting (only in "All Players" tab)
- Updated help.html with global leaderboard section

*** refactor: comprehensive CSS cleanup
- Removed ~240 lines of dead code (unused modal section)
- Fixed accessibility: focus contrast (0.1→0.3), touch targets (32px→44px), keyboard navigation
- Added missing dynamic class styles (.flashcard.revealed, .leaderboard-row variants)
- Added @media (hover: hover) for touch device compatibility
- Converted 12 hardcoded colors to CSS variables with 13 overlay variants
- Converted 24+ hardcoded pixels to CSS variables (spacing, dimensions)
- Final: 24 color variables, consistent design tokens throughout

** 2025/Nov/16
*** claude code issues: service worker duplicate update notification
Claude Code was unable to fix the issue with service worker. I had to read about it on https://github.com/deanhume/pwa-update-available

*** fix: service worker update notification showing twice
Fixed duplicate notification by adding boolean flag to prevent race conditions in statechange event handler.

*** fix: manifest.json PWA compliance issues
- Changed start_url from "/" to "./" for subdirectory hosting compatibility
- Fixed SVG icon purpose (removed invalid "maskable" for SVG)
- Added lang: "en" field

*** feat: show topic name in leaderboard header
TOP 10 SCORES now displays topic emoji and name for better context.

*** feat: backend API validation
- Added allowed topics whitelist (cosmetics, infancy, pregnancy, toiletries, dev-short)
- Added max score validation (10000) to prevent impossible scores
- Improved error handling - server error messages now displayed to user

*** refactor: reorganize project structure
Moved files to conventional directories:
- assets/icons/ - favicon and app icons
- scripts/ - bump-version.js, generate-manifest.js
- docs/ - CLAUDE.md, IDEAS.org, JOURNAL.org
Updated all references in manifest.json, sw.js, index.html, package.json

*** refactor: optimize data loading
Changed data.js to conditionally import dev topics only when ?dev=true, reducing production bundle size.

*** refactor: remove dead code
Removed unused functions: renderLeaderboardScores, clearProgress, exportData, importData, getTopicFailedWords (~88 lines)

** 2025/Nov/14
*** feat: add global leaderboard API with offline sync and modal submission UI
** 2025/Oct/30
*** fix: service worker infinite reload loop caused by unregistering on every page load
